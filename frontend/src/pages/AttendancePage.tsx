    ...
    <div className="space-y-6">
        ...
        <div className="flex gap-2">
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${
              view === 'all' ? 'bg-zinc-900 text-white' : 'bg-zinc-100 text-zinc-600 hover:bg-zinc-200'
            }`}
          >
          ...
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${
              view === 'late' ? 'bg-amber-500 text-white' : 'bg-amber-50 text-amber-600 hover:bg-amber-100'
            }`}
          >
      ...
      <div className="bg-white rounded-xl border border-zinc-200 p-4 flex flex-wrap items-end gap-3">
        <div className="flex-1 min-w-[150px]">
          ...
          <select
            value={filterEmp}
            onChange={(e) => setFilterEmp(Number(e.target.value))}
        ...
        <button
          onClick={() => { setFilterEmp(0); setDateFrom(''); setDateTo('') }}
          className="px-3 py-2 border border-zinc-200 rounded-lg text-sm text-zinc-500 hover:bg-zinc-50 transition-colors"
        >
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-zinc-100 bg-zinc-50/50">
                ...
                <th className="text-center px-5 py-3 font-medium text-zinc-500 text-xs uppercase tracking-wider">Estado / Justificación</th>
                ...
                return (
                  <tr key={rec.id} className="border-b border-zinc-50 hover:bg-zinc-50/50 transition-colors">
                    <td className="px-5 py-3 font-medium text-zinc-800">
                      {rec.employee?.name || `ID: ${rec.employee_id}`}
                    </td>
                    <td className="px-5 py-3 text-zinc-500">
                      {new Date(rec.date + 'T12:00:00').toLocaleDateString('es-MX')}
                    </td>
                    <td className="px-5 py-3 text-center">
                      <span className="inline-flex items-center gap-1 text-green-700">
                        <Play size={12} /> {formatTime(rec.entry_time)}
                      </span>
                    </td>
                    <td className="px-5 py-3 text-center">
                      <span className="inline-flex items-center gap-1 text-red-700">
                        <Square size={12} /> {formatTime(rec.exit_time)}
                      </span>
                    </td>
                    <td className="px-5 py-3 text-center">
                      <span className="inline-flex items-center gap-1 text-amber-700">
                        <Coffee size={12} />
                        {rec.break_start
                          ? `${formatTime(rec.break_start)} - ${formatTime(rec.break_end)}`
                          : '—'}
                      </span>
                    </td>
                        ...
                        editingId === rec.id ? (
                          <div className="flex items-center gap-1 justify-center">
                            <input
                              ...
                              className="px-2 py-1 border border-amber-300 rounded text-xs w-40 focus:outline-none focus:ring-1 focus:ring-amber-400"
                              placeholder="Justificación..."
                              autoFocus
                            />
                            <button onClick={() => handleSaveJustification(rec.id)} className="p-1 text-green-600 hover:bg-green-50 rounded">
                              <Check size={14} />
                            </button>
                            <button onClick={() => setEditingId(null)} className="p-1 text-zinc-400 hover:bg-zinc-100 rounded">
                              <X size={14} />
                            </button>
                          </div>
                        ) : (
                          <span
                            className="inline-flex items-center gap-1 text-amber-600 text-xs bg-amber-50 px-2 py-1 rounded-full cursor-pointer hover:bg-amber-100 transition-colors"
                            onClick={() => { setEditingId(rec.id); setEditJustification(rec.justification || '') }}
                          >
                            <AlertTriangle size={12} /> Tarde
                            {rec.justification && <span className="text-amber-500">· {rec.justification}</span>}
              ...
              {filtered.length === 0 && (
                <tr>
                  <td colSpan={8} className="px-5 py-12 text-center text-zinc-400 text-sm">
                    {view === 'late' ? 'No hay tardanzas registradas' : 'No hay registros de asistencia'}
                  </td>
                </tr>
              )}
            </tbody>
